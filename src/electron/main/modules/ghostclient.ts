...
bdFolder = path.join(bdFolder, "GhostClient") + "/";
...
export default class GhostClient {
    ...
    static async injectRenderer(browserWindow: BrowserWindow) {
        const location = path.join(__dirname, "ghostclient.js");
        ...
        const success = await browserWindow.webContents.executeJavaScript(`
            ...
            //# sourceURL=ghostclient/ghostclient.js
        `);
    ...
    static setup(browserWindow: BrowserWindow) {
        ...
        process.env.GHOSTCLIENT_DATA_PATH = bdFolder;
        process.env.GHOSTCLIENT_DATA_PATH = bdFolder;
        ...
        if (electron.app.setAsDefaultProtocolClient("ghostclient")) {
            // If application was opened via protocol, set process.env.GHOSTCLIENT_PROTOCOL
            const protocol = process.argv.find((arg) => arg.startsWith("ghostclient://"));
            if (protocol) {
                process.env.GHOSTCLIENT_PROTOCOL = protocol;
            }
            ...
            electron.app.on("second-instance", (_, argv) => {
                ...
                const url = argv.find((arg) => arg.startsWith("ghostclient://"));
    ...
    static disableMediaKeys() {
        if (!GhostClient.getSetting("general", "mediaKeys")) return;
...
if (GhostClient.getSetting("developer", "reactDevTools")) {
...
Object.defineProperty(global, "appSettings", {
    set(setting) {
        ...
        if (GhostClient.getSetting("window", "removeMinimumSize")) {
