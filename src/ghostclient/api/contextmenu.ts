...
// https://github.com/doggybootsy/vx/blob/main/packages/mod/src/ghostclient/context-menu.tsx
...
class MenuPatcher {
    private static async contextMenuFixForLucide() {
        ...
        DOMManager.injectStyle("bd-lucide-context-menu-fix", `
            :where(.${menuClasses!.colorDefault}, ${menuClasses!.colorDanger}).${menuClasses!.focused}:not(.${menuClasses!.checkboxContainer}) .lucide:not(.lucide-ghostclient) path {
                fill: revert;
            }
        `);
    }
