...
import type {BdWebAddon} from "ghostclientweb";
...
export class CoreUpdater {
    ...
    static async checkForStable(ignoreVersion = false) {
        const resp = await fetch(`https://api.github.com/repos/GhostClient/GhostClient/releases/latest`, {
            method: "GET",
            headers: {
                "Accept": "application/json",
                "Content-Type": "application/json",
                "User-Agent": "GhostClient Updater"
            }
        });
    ...
    static async checkForCanary(ignoreVersion = false) {
        const resp = await fetch(`https://api.github.com/repos/GhostClient/GhostClient/releases`, {
            method: "GET",
            headers: {
                "Accept": "application/json",
                "Content-Type": "application/json",
        ...
        const asset = data?.assets.find(a => a.name === "ghostclient.asar");
        if (!data || !asset) {
    ...
    static async update() {
        try {
            const asar = this.apiData.assets.find(a => a.name === "ghostclient.asar");
            if (!asar) return;
            ...
            const asarPath = path.join(Config.get("dataPath"), "ghostclient.asar");
