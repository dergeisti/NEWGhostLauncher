...
export default {
    ...
    execute: async (data, {channel}) => {
        ...
        if (action === "share") {
           ...
           return {content: "Unable to attach your Custom CSS as a file. Please report this issue to GhostClient's [GitHub](https://github.com/GhostClient/GhostClient) if no one else has already done so!"};
        }
    }
};