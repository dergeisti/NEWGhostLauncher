...
const PROTOCOL_REGEX = /^<ghostclient:\/\/(?:(?:theme|plugin|addon)s?|store)\/([^/\s]+)\/?>/i;
const APP_PROTOCOL_REGEX = /^ghostclient:\/\/(?:(?:theme|plugin|addon)s?|store)\/([^/]+)\/?$/i;

const ADDON_REGEX = new RegExp([
    PROTOCOL_REGEX.source.slice(1),
    /https?:\/\/ghostclient\.app\/(?:theme|plugin)(?:\/([^/\s]+)\/?|\?id=(\d+))/.source
].join("|"), "gi");
...
function extractAddonLinks(text, max = Infinity) {
    ...
    while ((exec = ADDON_REGEX.exec(text))) {
        // if https://ghostclient.app/type/id not <https://ghostclient.app/type/id>
        // if <ghostclient://addon/id> not ghostclient://addon/id
...
export default new class AddonStoreBuiltin extends Builtin {
    ...
    async enabled() {
        ...
        this.extractDiscordProtocolList().push("ghostclient:");
    }
        ...
        this.before(module, key, (_, args) => {
            if (args[0].href) {
                ...
                if (url.host === Web.hostname && url.pathname.toLowerCase() === "/download" && !isNaN(id)) {
                    ...
                    args[0].href = `ghostclient://store/${id}`;
    ...
    private extractDiscordProtocolList() {
        ...
        link.parse(["", "link", "ghostclient://foo/bar"]);
    ...
    async disabled() {
        ...
        const index = list.indexOf("ghostclient:");
        if (index !== -1) {
            list.splice(index, 1);
        }
